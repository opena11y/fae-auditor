...
(function (global, factory) {
        ...
        $('table.sortable').each(function () {
            ...
            $this.find('> thead tr').each(function (rowIndex) {
                $(this).find('th').each(function (columnIndex) {
                    ...
                    if ($header.attr('data-defaultsort') !== 'disabled') {
                      $header.attr('aria-sort', 'none');
                      var label = $header.text();
                      if (label.length < 3 && $header.attr('title')) {
                        label = $header.attr('title');
                      }
                      label = 'Sort by ' + label;
                      $header.html('<div role="button" tabindex="0" aria-label="' + label + '">' + $header.text() + '</div>');
                    }
                });
            });
    ...
    // Add click event to table header (keyboard support for accessibility)
    $document.on('keydown', 'table.sortable>thead th[data-defaultsort!="disabled"] [role=button]', function (e) {
        if (event.which === 32 || event.which === 13) {
          sortByColumn(this.parentNode);
        }
    });

    // Add focus event to table header (keyboard support for accessibility)
    $document.on('focus', 'table.sortable>thead th[data-defaultsort!="disabled"] [role=button]', function (e) {
        $(this).parent().addClass('focus');
    });

    // Add blur event to table header (keyboard support for accessibility)
    $document.on('blur', 'table.sortable>thead th[data-defaultsort!="disabled"] [role=button]', function (e) {
    ...
    function doSort($this, $table) {
        ...
        if (bsSort[sortKey] === 'desc') {
            ...
            $this.find('span.sign').attr('aria-sort', 'ascending');
